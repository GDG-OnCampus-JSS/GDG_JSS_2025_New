"use client";
import Image from "next/image";
import { useState } from "react";

export default function Header() {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  return (
    <header className="w-full px-[8vw] py-4 inline-flex justify-between items-center overflow-hidden">
      <div className="flex justify-start items-center gap-1">
        <Image
          src="/icons/gdsclogo.svg"
          alt="gdsc logo"
          width={32}
          height={32}
        />
        <span className="justify-start text-neutral-700 text-base font-normal font-['Product_Sans'] cursor-pointer">
          GDG JSSATEN
        </span>
      </div>
      <button
        className="md:hidden ml-[30vw]"
        onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
      >
        <svg
          width="28"
          height="28"
          viewBox="0 0 14 10"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="text-black"
        >
          <path
            d="M1 0H6C6.26522 0 6.51957 0.105357 6.70711 0.292893C6.89464 0.48043 7 0.734784 7 1C7 1.26522 6.89464 1.51957 6.70711 1.70711C6.51957 1.89464 6.26522 2 6 2H1C0.734784 2 0.48043 1.89464 0.292893 1.70711C0.105357 1.51957 0 1.26522 0 1C0 0.734784 0.105357 0.48043 0.292893 0.292893C0.48043 0.105357 0.734784 0 1 0ZM8 8H13C13.2652 8 13.5196 8.10536 13.7071 8.29289C13.8946 8.48043 14 8.73478 14 9C14 9.26522 13.8946 9.51957 13.7071 9.70711C13.5196 9.89464 13.2652 10 13 10H8C7.73478 10 7.48043 9.89464 7.29289 9.70711C7.10536 9.51957 7 9.26522 7 9C7 8.73478 7.10536 8.48043 7.29289 8.29289C7.48043 8.10536 7.73478 8 8 8ZM1 4H13C13.2652 4 13.5196 4.10536 13.7071 4.29289C13.8946 4.48043 14 4.73478 14 5C14 5.26522 13.8946 5.51957 13.7071 5.70711C13.5196 5.89464 13.2652 6 13 6H1C0.734784 6 0.48043 5.89464 0.292893 5.70711C0.105357 5.51957 0 5.26522 0 5C0 4.73478 0.105357 4.48043 0.292893 4.29289C0.48043 4.10536 0.734784 4 1 4Z"
            fill="currentColor"
          />
        </svg>
      </button>

      <div className="hidden px-1 relative md:flex justify-start items-center gap-5">
        <button className=" md:block px-3 py-2 rounded-[36.80px] flex justify-center items-center gap-2.5 hover:text-[#1A73E8] cursor-pointer">
          <label className="justify-start text-Primary text-base font-medium font-['Product_Sans_Medium'] cursor-pointer">
            Home
          </label>
        </button>
        <button className=" md:block px-3 py-2 rounded-[36.80px] flex justify-center items-center gap-2.5 hover:text-[#1A73E8] cursor-pointer">
          <label className="justify-start text-Secondary text-base font-normal font-['Product_Sans'] cursor-pointer">
            Projects
          </label>
        </button>
        <button className=" md:block px-3 py-2 rounded-[36.80px] flex justify-center items-center gap-2.5 hover:text-[#1A73E8] cursor-pointer">
          <label className="justify-start text-Secondary text-base font-normal font-['Product_Sans'] cursor-pointer">
            Team
          </label>
        </button>
        <button className=" md:block px-3 py-2 rounded-[36.80px] flex justify-center items-center gap-2.5 hover:text-[#1A73E8] cursor-pointer">
          <label className="justify-start text-Secondary text-base font-normal font-['Product_Sans'] cursor-pointer">
            Events
          </label>
        </button>
        <button className=" md:block px-3 py-2 rounded-[36.80px] flex justify-center items-center gap-2.5 hover:text-[#1A73E8] cursor-pointer">
          <label className="justify-start text-Secondary text-base font-normal font-['Product_Sans'] cursor-pointer">
            Alumni
          </label>
        </button>
        <button className=" md:block h-10 px-5 py-2 bg-blue-600 rounded-[44px] flex justify-center items-center gap-2.5 cursor-pointer">
          <label className="text-center justify-center text-white text-base font-normal font-['Product_Sans'] cursor-pointer">
            Connect
          </label>
        </button>
      </div>

      {isMobileMenuOpen && (
        <div className="absolute top-16 right-6 bg-white shadow-lg rounded-lg flex flex-col gap-4 p-6 lg:hidden">
          <button
            onClick={() => {
              setIsMobileMenuOpen(false);
            }}
            className="hover:text-[#1A73E8]"
          >
            <label className="justify-start text-Primary text-base font-medium font-['Product_Sans_Medium'] cursor-pointer">
              Home
            </label>
          </button>
          <button
            onClick={() => {
              setIsMobileMenuOpen(false);
            }}
            className="hover:text-[#1A73E8]"
          >
            <label className="justify-start text-Secondary text-base font-normal font-['Product_Sans'] cursor-pointer">
              Projects
            </label>
          </button>
          <button
            onClick={() => {
              setIsMobileMenuOpen(false);
            }}
            className="hover:text-[#1A73E8]"
          >
            <label className="justify-start text-Secondary text-base font-normal font-['Product_Sans'] cursor-pointer">
              Team
            </label>
          </button>
          <button
            onClick={() => {
              setIsMobileMenuOpen(false);
            }}
            className="hover:text-[#1A73E8]"
          >
            <label className="justify-start text-Secondary text-base font-normal font-['Product_Sans'] cursor-pointer">
              Events
            </label>
          </button>
          <button
            onClick={() => setIsMobileMenuOpen(false)}
            className="hover:text-[#1A73E8]"
          >
            <label className="justify-start text-Secondary text-base font-normal font-['Product_Sans'] cursor-pointer">
              Alumni
            </label>
          </button>
          <button
            onClick={() => {
              setIsMobileMenuOpen(false);
            }}
            className="hover:text-[#1A73E8]"
          >
            <label className="text-Secondary justify-center text-base font-normal font-['Product_Sans'] cursor-pointer">
              Connect
            </label>
          </button>
        </div>
      )}
    </header>
  );
}
